class CarrDemo < ApplicationRecord
  ...
  scope :listings, -> {select("carr_demo.*,
  ...
  ).joins('LEFT JOIN carriers ON carriers.mc_number = carr_demo."MC#"').joins("LEFT JOIN carrier_contacts AS contacts ON contacts.id = carriers.poc_id").joins("LEFT JOIN users AS relationship_owner_user ON relationship_owner_user.id = carriers.relationship_owner")}
