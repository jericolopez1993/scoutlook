class Carrier < ApplicationRecord
  ...
  scope :listings, -> {select("carriers.*, (DATE(carriers.created_at) = DATE(CURRENT_DATE - 21)) AS three_weeks_lapse, contacts.primary_phone, contacts.primary_phone_type, contacts.primary_extension_number, contacts.primary_eligible_texting, contacts.secondary_phone, contacts.secondary_phone_type, contacts.secondary_extension_number, contacts.secondary_eligible_texting, contacts.email as contact_email, locations.city, locations.address, locations.state, locations.country, locations.loc_type, (SELECT date_opened FROM activities WHERE activities.carrier_id = carriers.id ORDER BY created_at DESC LIMIT 1) as date_opened, CONCAT(relationship_owner_user.first_name, ' ', relationship_owner_user.last_name) as relationship_owner_name, CONCAT(carrier_setup_user.first_name, ' ', carrier_setup_user.last_name) as carrier_setup_name, CONCAT(contacts.first_name, ' ', contacts.last_name) as poc_name, (SELECT cnactivities.campaign_name FROM activities AS cnactivities WHERE cnactivities.carrier_id =  carriers.id ORDER BY created_at DESC LIMIT 1) AS campaign_name, (SELECT atactivities.activity_type FROM activities AS atactivities WHERE atactivities.carrier_id =  carriers.id ORDER BY created_at DESC LIMIT 1) AS activity_type, (SELECT sactivities.status FROM activities AS sactivities WHERE sactivities.carrier_id =  carriers.id ORDER BY created_at DESC LIMIT 1) AS activity_status").joins("LEFT JOIN users AS carrier_setup_user ON carrier_setup_user.id = carriers.carrier_setup").joins("LEFT JOIN users AS relationship_owner_user ON relationship_owner_user.id = carriers.relationship_owner").joins("LEFT JOIN carrier_locations AS locations ON locations.id = carriers.head_office").joins("LEFT JOIN carrier_contacts AS contacts ON contacts.id = carriers.poc_id")}

  scope :overall, ->(id) {where(:id => id).select("carriers.*, latest_data.\"Tier\" AS current_tier, latest_data.loadsh_num AS last_month, latest_data.loadsh_num_6mon AS last_6_months, (SELECT (now()::date - ship_date_1::date) FROM mc_latest_date WHERE mcnum = carriers.mc_number ORDER BY ship_date_1 DESC LIMIT 1) AS load_days, (DATE(carriers.created_at) = DATE(CURRENT_DATE - 21)) AS three_weeks_lapse, contacts.primary_phone, contacts.primary_phone_type, contacts.primary_extension_number, contacts.primary_eligible_texting, contacts.secondary_phone, contacts.secondary_phone_type, contacts.secondary_extension_number, contacts.secondary_eligible_texting, contacts.email as contact_email, locations.city, locations.address, locations.state, locations.country, locations.loc_type, (SELECT id FROM carrier_lanes WHERE carrier_lanes.carrier_id = carriers.id ORDER BY lane_priority DESC, created_at DESC LIMIT 1) AS lane_id, (SELECT lane_destination FROM carrier_lanes WHERE carrier_lanes.carrier_id = carriers.id ORDER BY lane_priority DESC, created_at DESC LIMIT 1) as lane_destination, (SELECT lane_origin FROM carrier_lanes WHERE carrier_lanes.carrier_id = carriers.id ORDER BY lane_priority DESC, created_at DESC LIMIT 1) as lane_origin, (SELECT date_opened FROM activities WHERE activities.carrier_id = carriers.id ORDER BY created_at DESC LIMIT 1) as date_opened, CONCAT(relationship_owner_user.first_name, ' ', relationship_owner_user.last_name) as relationship_owner_name, CONCAT(carrier_setup_user.first_name, ' ', carrier_setup_user.last_name) as carrier_setup_name, CONCAT(contacts.first_name, ' ', contacts.last_name) as poc_name, (SELECT created_at FROM audits WHERE ((auditable_type = 'Carrier' AND auditable_id IN (carriers.id)) OR (auditable_type = 'CarrierCompany' AND auditable_id IN (SELECT carrier_companies.id FROM carrier_companies WHERE carrier_companies.carrier_id = carriers.id)) OR (auditable_type = 'CarrierContact' AND auditable_id IN (SELECT carrier_contacts.id FROM carrier_contacts WHERE carrier_contacts.carrier_id = carriers.id)) OR (auditable_type = 'CarrierLane' AND auditable_id IN (SELECT carrier_lanes.id FROM carrier_lanes WHERE carrier_lanes.carrier_id = carriers.id)) OR (auditable_type = 'CarrierLocation' AND auditable_id IN (SELECT carrier_locations.id FROM carrier_locations WHERE carrier_locations.carrier_id = carriers.id)) OR (auditable_type = 'Activity' AND auditable_id IN (SELECT activities.id FROM activities WHERE activities.carrier_id = carriers.id)) OR (auditable_type = 'Rate' AND auditable_id IN (SELECT rates.id FROM rates WHERE rates.carrier_id = carriers.id)) OR (auditable_type = 'Reminder' AND auditable_id IN (SELECT reminders.id FROM reminders WHERE reminders.carrier_id = carriers.id))) ORDER BY created_at DESC LIMIT 1) AS last_activity_date, (SELECT cnactivities.campaign_name FROM activities AS cnactivities WHERE cnactivities.carrier_id =  carriers.id ORDER BY created_at DESC LIMIT 1) AS campaign_name, (SELECT atactivities.activity_type FROM activities AS atactivities WHERE atactivities.carrier_id =  carriers.id ORDER BY created_at DESC LIMIT 1) AS activity_type, (SELECT sactivities.status FROM activities AS sactivities WHERE sactivities.carrier_id =  carriers.id ORDER BY created_at DESC LIMIT 1) AS activity_status").joins("LEFT JOIN users AS carrier_setup_user ON carrier_setup_user.id = carriers.carrier_setup").joins("LEFT JOIN users AS relationship_owner_user ON relationship_owner_user.id = carriers.relationship_owner").joins("LEFT JOIN carrier_locations AS locations ON locations.id = carriers.head_office").joins("LEFT JOIN carrier_contacts AS contacts ON contacts.id = carriers.poc_id").joins("LEFT JOIN carrier_contacts AS contacts ON contacts.id = carriers.poc_id").joins('LEFT JOIN mc_latest_date AS latest_data ON latest_data.mcnum = carriers.mc_number').limit(1)}

  scope :mine, ->(id) {where(:relationship_owner => id).select("carriers.*, latest_data.\"Tier\" AS current_tier, latest_data.loadsh_num AS last_month, latest_data.loadsh_num_6mon AS last_6_months, (SELECT (now()::date - ship_date_1::date) FROM mc_latest_date WHERE mcnum = carriers.mc_number ORDER BY ship_date_1 DESC LIMIT 1) AS load_days, (DATE(carriers.created_at) = DATE(CURRENT_DATE - 21)) AS three_weeks_lapse, contacts.primary_phone, contacts.primary_phone_type, contacts.primary_extension_number, contacts.primary_eligible_texting, contacts.secondary_phone, contacts.secondary_phone_type, contacts.secondary_extension_number, contacts.secondary_eligible_texting, contacts.email as contact_email, locations.city, locations.address, locations.state, locations.country, locations.loc_type, (SELECT id FROM carrier_lanes WHERE carrier_lanes.carrier_id = carriers.id ORDER BY lane_priority DESC, created_at DESC LIMIT 1) AS lane_id, (SELECT lane_destination FROM carrier_lanes WHERE carrier_lanes.carrier_id = carriers.id ORDER BY lane_priority DESC, created_at DESC LIMIT 1) as lane_destination, (SELECT lane_origin FROM carrier_lanes WHERE carrier_lanes.carrier_id = carriers.id ORDER BY lane_priority DESC, created_at DESC LIMIT 1) as lane_origin, (SELECT date_opened FROM activities WHERE activities.carrier_id = carriers.id ORDER BY created_at DESC LIMIT 1) as date_opened, CONCAT(relationship_owner_user.first_name, ' ', relationship_owner_user.last_name) as relationship_owner_name, CONCAT(carrier_setup_user.first_name, ' ', carrier_setup_user.last_name) as carrier_setup_name, CONCAT(contacts.first_name, ' ', contacts.last_name) as poc_name, (SELECT created_at FROM audits WHERE ((auditable_type = 'Carrier' AND auditable_id IN (carriers.id)) OR (auditable_type = 'CarrierCompany' AND auditable_id IN (SELECT carrier_companies.id FROM carrier_companies WHERE carrier_companies.carrier_id = carriers.id)) OR (auditable_type = 'CarrierContact' AND auditable_id IN (SELECT carrier_contacts.id FROM carrier_contacts WHERE carrier_contacts.carrier_id = carriers.id)) OR (auditable_type = 'CarrierLane' AND auditable_id IN (SELECT carrier_lanes.id FROM carrier_lanes WHERE carrier_lanes.carrier_id = carriers.id)) OR (auditable_type = 'CarrierLocation' AND auditable_id IN (SELECT carrier_locations.id FROM carrier_locations WHERE carrier_locations.carrier_id = carriers.id)) OR (auditable_type = 'Activity' AND auditable_id IN (SELECT activities.id FROM activities WHERE activities.carrier_id = carriers.id)) OR (auditable_type = 'Rate' AND auditable_id IN (SELECT rates.id FROM rates WHERE rates.carrier_id = carriers.id)) OR (auditable_type = 'Reminder' AND auditable_id IN (SELECT reminders.id FROM reminders WHERE reminders.carrier_id = carriers.id))) ORDER BY created_at DESC LIMIT 1) AS last_activity_date, (SELECT cnactivities.campaign_name FROM activities AS cnactivities WHERE cnactivities.carrier_id =  carriers.id ORDER BY created_at DESC LIMIT 1) AS campaign_name, (SELECT atactivities.activity_type FROM activities AS atactivities WHERE atactivities.carrier_id =  carriers.id ORDER BY created_at DESC LIMIT 1) AS activity_type, (SELECT sactivities.status FROM activities AS sactivities WHERE sactivities.carrier_id =  carriers.id ORDER BY created_at DESC LIMIT 1) AS activity_status").joins("LEFT JOIN users AS carrier_setup_user ON carrier_setup_user.id = carriers.carrier_setup").joins("LEFT JOIN users AS relationship_owner_user ON relationship_owner_user.id = carriers.relationship_owner").joins("LEFT JOIN carrier_locations AS locations ON locations.id = carriers.head_office").joins("LEFT JOIN carrier_contacts AS contacts ON contacts.id = carriers.poc_id").joins('LEFT JOIN mc_latest_date AS latest_data ON latest_data.mcnum = carriers.mc_number')}
