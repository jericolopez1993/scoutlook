class ApplicationController < ActionController::Base
  ...
  def get_logs_and_reminders
    ...
    @reminders_today = Reminder.listings.where("reminders.action_taken = 0 AND (reminders.reminder_date >= date_trunc('day', current_date) AND reminders.reminder_date <= ?)", Time.now.strftime("%Y-%m-%d %H:%M"))
  end
