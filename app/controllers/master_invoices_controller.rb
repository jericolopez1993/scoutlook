class MasterInvoicesController < ApplicationController
          ...
          if @master_invoice.shipment_entry == "Single"
            ...
            if params[:master_invoice][:destination_location_id].present? || params[:master_invoice][:destination_address].present? || params[:master_invoice][:destination_country].present? ||  params[:master_invoice][:destination_state].present? ||  params[:master_invoice][:destination_postal].present? ||  params[:master_invoice][:destination_city].present?
              if is_numeric?(params[:master_invoice][:destination_location_id])
                ...
                location.client_id = @master_invoice.shipper_id
                location.save
        ...
        if @master_invoice.shipment_entry == "Single"
            ...
            if params[:master_invoice][:destination_location_id].present? || params[:master_invoice][:destination_address].present? || params[:master_invoice][:destination_country].present? ||  params[:master_invoice][:destination_state].present? ||  params[:master_invoice][:destination_postal].present? ||  params[:master_invoice][:destination_city].present?
              if is_numeric?(params[:master_invoice][:destination_location_id])
                ...
                location.city = params[:master_invoice][:destination_city]
                location.postal = params[:master_invoice][:destination_postal]
                location.client_id = params[:master_invoice][:shipper_id]
                location.save
                @destination_location_id = location.id
              end
            end
  ...
  private
    ...
    params[:master_invoice][:audit_comment] = params[:master_invoice][:invoice_audit_comment]
      params.require(:master_invoice).permit(:invoice_number, :shipment_type, :shipper_id, :carrier_id, :master_account, :single_invoice_date, :invoicing_period_start, :invoicing_period_end, :total_charge, :variance_approved, :shipment_entry, :own_status, :audit_comment)
    end
    ...
    def shipment_params
      ...
      params[:master_invoice][:audit_comment] = params[:master_invoice][:shipment_audit_comment]
