...
s_a[2]="AL|AK|AZ|AR|CA|CO|CT|DE|FL|GA|HI|ID|IL|IN|IA|KS|KY|LA|ME|MD|MA|MI|MN|MS|MO|MT|NE|NH|NJ|NV|NY|NY-Brooklyn|NY-Bronx|NC|ND|OH|OK|OR|PA|RI|SC|SD|TN|TX|UT|VT|VA|WA|WV|WI|WY|US-Northeast|US-Northeast (No Bronx)|US-Southeast|US-Northwest|US-Midwest";
...
function populateCountries(countryElementId, stateElementId){
	...
	for (var i=0; i<country_arr.length; i++) {
		if (countryElement.length == 3) {
			opt = new Option('','');
			opt.className = 'separator';
			opt.disabled = true;
			countryElement.options[countryElement.length] = opt;
		}
