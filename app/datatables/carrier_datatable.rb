class CarrierDatatable < AjaxDatatablesRails::ActiveRecord
  ...
  def view_columns
    ...
    @view_columns ||= {
      ...
      interview: { source: "Carrier.interview" },
      wolfbyte: { source: "Carrier.wolfbyte" },
      ...
      c_mc_latest_date_tier: { source: "Carrier.c_mc_latest_date_tier" },
    ...
    records.map do |record|
      {
        ...
        relationship_owner_name: record.relationship_owner ? (record['relationship_owner_name'].blank? ? '(no name)' : "#{link_to(covert_initials(record['relationship_owner_name']), user_path(:id => record['relationship_owner']))}".html_safe) : '',
        ...
        c_mc_latest_date_tier: generate_rank_text(record.c_mc_latest_date_tier).html_safe,
